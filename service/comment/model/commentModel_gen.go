...
var (
	...
	cacheForumCommentIdPrefix = "cache:forum:comment:id:"
)
...
func (m *defaultCommentModel) Delete(ctx context.Context, id int64) error {
	forumCommentIdKey := fmt.Sprintf("%s%v", cacheForumCommentIdPrefix, id)
...
func (m *defaultCommentModel) FindOne(ctx context.Context, id int64) (*Comment, error) {
	forumCommentIdKey := fmt.Sprintf("%s%v", cacheForumCommentIdPrefix, id)
	var resp Comment
	err := m.QueryRowCtx(ctx, &resp, forumCommentIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultCommentModel) Insert(ctx context.Context, data *Comment) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.UserId, data.Desc, data.DeletedAt)
	}, forumCommentIdKey)
	return ret, err
}
...
func (m *defaultCommentModel) Update(ctx context.Context, data *Comment) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.UserId, data.Desc, data.DeletedAt, data.Id)
	}, forumCommentIdKey)
	return err
}

func (m *defaultCommentModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheForumCommentIdPrefix, primary)
}
